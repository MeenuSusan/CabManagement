...
<html lang="en">
  ...
  ?>
    ...
    <?php
                    $res=mysqli_query($con,"SELECT `id`,`status` FROM `register` where `id`= $user_session_id");
                    while($r=mysqli_fetch_array($res)){
                      $vehicle_approvel_status=$r['status'];
                      ?>
        <table >
            <tr>
                <th>
                Vehicle approvel Status
                </th>
                <td></td>
                <td>
                <?php 
                         
                         if($vehicle_approvel_status==1){?>
                          <button class="btn1 btn-success btn-block">Approved</button><?php
                         }
                         elseif($vehicle_approvel_status==0){?>
                           <button class="btn1 btn-danger btn-block">Rejected</button><?php
                         }
                         elseif($vehicle_approvel_status==2){?>
